 ...
 * Wrapped in an IIFE so each injection gets its own function scope,
 * preventing const redeclaration errors when injected into existing tabs.
 */
(() => {
  if (window.__vidurActive) return; // already running — skip re-init
  ...
  // Exposed on window so chrome.scripting.executeScript can call it from popup
  window.__vidurExtractPageProfile = function () {
    ...
    profile.rootClasses = [...document.documentElement.classList,
